...
// ⚡ FIX: Activity import
import { Navigation, X, MapPin, Play, Activity, ChevronUp, Zap, Route, Mountain, Loader2 } from 'lucide-react';
  ...
  if (routeOptions && routeOptions.options) {
    ...
    return (
      <div className={`absolute bottom-0 left-0 w-full bg-zinc-900 border-t border-zinc-800 shadow-2xl z-[60] transition-all duration-300 ease-in-out rounded-t-3xl ${isExpanded ? 'pb-8' : 'pb-32'}`}>
        <button onClick={() => setIsExpanded(!isExpanded)} className="w-full flex justify-center pt-3 pb-1 cursor-pointer">
          <div className="w-12 h-1.5 bg-zinc-700 rounded-full"></div>
        </button>

        {isExpanded && (
          <div className="px-6 mb-4 space-y-2 animate-in slide-in-from-bottom-5">
            <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-3">Select Route Type</p>
            {routeOptions.options.map((opt) => (
              <button
                key={opt.id}
                onClick={() => { routeOptions.onSelect(opt.id); setIsExpanded(false); }}
                className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all ${
                  routeOptions.selectedId === opt.id ? 'bg-zinc-800 border-green-500/50 shadow-lg' : 'bg-transparent border-zinc-800 hover:bg-zinc-800'
                }`}
              >
                <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${
                        opt.tag === 'Fun' || opt.tag === 'Twisty' ? 'bg-purple-500/10 text-purple-400' :
                        opt.tag === 'Fastest' ? 'bg-green-500/10 text-green-400' : 'bg-zinc-700/50 text-zinc-400'
                    }`}>
                        {opt.tag === 'Fun' || opt.tag === 'Twisty' ? <Mountain size={18} /> :
                         opt.tag === 'Fastest' ? <Zap size={18} /> : <Route size={18} />}
                    </div>
                    <div className="text-left">
                        <div className="font-bold text-white">{opt.tag}</div>
                        <div className="text-xs text-zinc-500">{opt.stats.km} km • {opt.stats.vibe}</div>
                    </div>
                </div>
                <div className="text-xl font-bold text-white">{opt.stats.mins} <span className="text-xs font-normal text-zinc-500">min</span></div>
              </button>
            ))}
          </div>
        )}
        ...
        {!isExpanded && (
            <div className="px-6 mb-6">
                <div className="flex justify-between items-end mb-6 cursor-pointer" onClick={() => setIsExpanded(true)}>
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <span className={`text-xs font-bold uppercase px-2 py-0.5 rounded border ${
                            selectedRoute.tag === 'Fun' ? 'border-purple-500 text-purple-400' : 'border-green-500 text-green-400'
                        }`}>
                            {selectedRoute.tag}
                        </span>
                        <ChevronUp size={14} className="text-zinc-500 animate-bounce" />
                    </div>
                ...
                <div className={`px-3 py-1 rounded-full border ${
                    ...
                    <Activity size={14} />
                    <span className="text-xs font-bold">{selectedRoute.stats.sinuosity}</span>
                </div>
                </div>
                
                <div className="flex gap-3">
                <button onClick={onClose} className="p-4 bg-zinc-800 rounded-2xl text-zinc-400 hover:text-white"><X size={24} /></button>
                <button onClick={handleStart} className="flex-1 bg-green-600 text-white font-bold text-lg py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-green-500 transition-all shadow-lg shadow-green-900/20">
